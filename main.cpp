...
{
  public:
    RES pack(std::string arrs[], int len)
    {
        RES r;
        int s = 1;
        for (int i = 0; i < len; i++)
...
class Database
{
  public:
    ...
    bool CreateSet(std::string Set_name, std::map<std::string, RES> kv)
    {
        ...
        lx.CreateSetBackUse(Set_name, kv);
        return true;
    }
    
    bool Update(std::string kxy, RES r, std::string path, std::string name){
     
        lx.Update(kxy,r,path,name);
...
int main()
{
    ...
    std::string arr1[2];
    std::string arr2[2];
    arr1[0]="shivxgvccansh";
    arr1[1]="cannon";
    arr2[0]="none";
    arr2[1]="learner";
    kd["name"] = hp.pack(arr1, 2);
    kd["behaviour"] = hp.pack(arr2, 2);
    db.CreateSet("SUCCESS", kd);
    ...
    f.arr[1] = "chill";
    f.arr[2] = "myendboi";
    db.Update("behaviour", f, "./Database/SUCCESS", "SUCCdygxdESS");
}